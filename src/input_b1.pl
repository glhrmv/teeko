...
read_position_to_b(Player, Board, Columm, Line) :-
	...
	CNumber is C - 65,
	LNumber is L - 48,
	read_position_to_check_b(Player, Board, CNumber, LNumber),
	Columm is CNumber,
	Line is LNumber.

read_position_to_b(Player, Board, _, _) :-
	write('Invalid position choice. Remember: '), nl,
	write('Columm: A to E'), nl,
	write('Line: 0 to 4'), nl,
	bot_1(Player, Board).

read_position_from_b(Player, Board, Columm, Line) :-
	...
	CNumber is C - 65,
	LNumber is L - 48,
	read_position_to_check_from_b(Player, Board, CNumber, LNumber),
	Columm is CNumber,
	Line is LNumber.
...
read_position_to_check_b(_Player, Board, Columm, Line) :-
	check_free_space(Board, Columm, Line, Value),
	Value = 'e'.

read_position_to_check_b(Player, Board, _Columm, _Line) :-
	write('There is already a marker on that position'), nl,
	bot_1(Player, Board).

read_position_to_check_from_b(Player, Board, Columm, Line) :-
	check_free_space(Board, Columm, Line, Value),
	player(Player, Piece),
	Value = Piece.
